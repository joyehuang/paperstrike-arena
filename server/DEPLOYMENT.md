...
当前已实现可本地联机的原型，尚未开通付费服务。前端 `/pvp` 在没有配置服务地址时显示准备中，原单人模式不受影响。

## 架构与范围

Vercel 托管静态前端；独立 Node.js + Colyseus 进程维护房间，通过 HTTPS/WSS 接入。首版每房间 2–4 人，废稿堆场、三分钟自由混战、准备/房主开局/结算/重开、四把枪和补给。服务器裁定移动、伤害、弹药、复活，30 Hz 同步，每次模拟分成四个物理子步。

移动端和电脑端使用独立匹配池，房间码加入也经过服务器校验。依据 User-Agent 和触控提示分类；这不能防止伪造设备信息，也无法可靠识别手机外接鼠标键盘。尚未实现占点、武器轮换和锁定武器规则。

客户端目前只有基本移动预测、位置纠正和远端插值；完整输入重放、射击延迟补偿、真实手机操作和高延迟体验仍需验证。不要把当前原型当作已完成竞技公平性验收的版本。
...
4. 健康检查 `/health` 应返回 `{"status":"ok","service":"paperstrike-pvp","protocol":1}`。使用平台实际分配的 HTTPS 地址，不要猜测服务域名。
...
免费 Render 服务在无入站 HTTP/WebSocket 消息 15 分钟后休眠，唤醒有等待，因此正式试玩建议付费常驻实例。费用以创建页为准；本次没有创建资源或产生这项账单。
