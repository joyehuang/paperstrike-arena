...
常驻服务已开通于 https://paperstrike-pvp.onrender.com，前端部署已接入。前端 `/pvp` 在没有配置服务地址时显示准备中，原单人模式不受影响。
...
客户端使用带序号的输入位置历史做确认位置纠正，昵称使用缓存的朝向相机贴图，HUD 显示 FPS 和 RTT；完整输入重放、射击延迟补偿、真实手机操作和高延迟体验仍需验证。不要把当前原型当作已完成竞技公平性验收的版本。
